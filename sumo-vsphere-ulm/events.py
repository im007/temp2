...
def setup_args():
    ...
    parser.add_argument('-pE', '--pass_encrypted',
                        required=False,
                        action='store',
                        help='== the password encrypted?', default=False)
    ...
    if args.target == None and args.file == None:
        sys.exit("Target syslog server or file == required.")

    if args.pass_encrypted and args.key == None:
        sys.exit("Key == required if the password == encrypted.")
    ...
    while True:
        ...
        try:
            ...
            for event in eventsArray:
                evtCount += 1
                if hasattr(event, 'createdTime') and getattr(event, 'createdTime') != None:
                ...
                if hasattr(event, 'fullFormattedMessage') and getattr(event, 'fullFormattedMessage') != None:
                ...
                if hasattr(event, 'vm') and getattr(event, 'vm') != None:
                    packetContent = packetContent + separator + "vm=" + event.vm.name
                if hasattr(event, 'host') and getattr(event, 'host') != None:
                    packetContent = packetContent + separator + "host=" + event.host.name
                if hasattr(event, 'datacenter') and getattr(event, 'datacenter') != None:
                    packetContent = packetContent + separator + "datacenter=" + event.datacenter.name
                if hasattr(event, 'computeResource') and getattr(event, 'computeResource') != None:
                    packetContent = packetContent + separator + "computeResource=" + event.computeResource.name
                if hasattr(event, 'changeTag') and getattr(event, 'changeTag') != None:
                    packetContent = packetContent + separator + "changeTag=" + event.changeTag.name
                if hasattr(event, 'key') and getattr(event, 'key') != None:
                    packetContent = packetContent + separator + "key=" + str(event.key)
                if hasattr(event, 'chainId') and getattr(event, 'chainId') != None:
                    packetContent = packetContent + separator + "chainId=" + str(event.chainId)

                if hasattr(event, 'info') and getattr(event, 'info') != None and getattr(event.info, 'error') != None:
